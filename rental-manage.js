...
function renderSelectedOrderHeader(order) {
  if (!order) {
    selectedOrderMeta.textContent = "Wybierz dokument z listy po lewej stronie.";
    selectedOrderStatus.textContent = "Brak wyboru";
...
async function receiveReturn() {
  ...
  if (selectedDraftItems.length !== order.items.length || selectedDraftItems.some((item) => (getOriginalQuantityMap(order).get(item.deviceCode) || 0) !== item.quantity)) {
...
async function refreshData(keepSelection = true) {
  const previousSelection = keepSelection ? selectedOrderId : null;
  await fetchInventory();
  await fetchRentalOrders();

  const nextSelection = previousSelection && rentalOrders.some((order) => order.id === previousSelection)
    ? previousSelection
    : rentalOrders[0]?.id || null;

  if (nextSelection) {
    selectOrder(nextSelection);
  } else {
    selectedOrderId = null;
    selectedDraftItems = [];
    fillSelectedOrderForm(null);
    renderOrdersList();
